...
outputDir_Admin <- "settings"
    ...
    #app--------------
    ui <- fluidPage(
        titlePanel(title = span(img(src = "flowlogo.png", height = 35) ),"1. Flow Kupa 2024"),
        # logout button
    ...
    server <- function(input, output, session) {
        ...
        output$sidebarpanel <- renderUI({
            ...
            if(credentials()$info[[3]]=="admin"){
                fluidPage(
                    ...
                    strong("Eredmények"),br(),
                    "Results",
                    DT::dataTableOutput("results", width = 300), tags$hr(),
                    strong("Megmászások és Zónák"),br(),
                    "Tops & zones",
                    DT::dataTableOutput("responses", width = 300), tags$hr(),
                    strong("Aktuális Pontok boulderenként (Top=1000/topok száma, Zóna=500/zónák száma)"),br(),
                    "Actual points for boulders (Top=1000/number of tops, Zone=500/number of zones)",
                    DT::dataTableOutput("points", width = 300), tags$hr()
                )
            }else{
                fluidPage(
                    tags$hr(),
                    ...
                    radioButtons("cat","Category",
                                 choiceNames = c("Lelkes","Kattant","Szikkadt","Masters","Veterán"),
                                 choiceValues =c("lelkes","kattant","szikkadt","masters","veteran"),
                                 selected = "lelkes",inline = T),tags$hr(),
                    column(3,
                           (if("B1" %in% settings()){
        ...
        output$results <- DT::renderDataTable({
            ...
            data3<- loadData() %>% 
                select(user,name,sex,cat,all_of(as.character(cn)))
            data2<-loadData() %>% 
                select(all_of(as.character(cn)))
            tops<-as.data.frame(ifelse(data2=="Top",1,0)) %>% colSums()
            zones<-as.data.frame(ifelse(data2=="Top",1,
